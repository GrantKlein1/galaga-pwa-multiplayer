...
assert(ids.length === 15, "15 roster bosses, got " + ids.length);
...
assert(meta(80).type === "seraph" && meta(80).tier === 1, "+1 cycle still starts at Seraph");
assert(meta(135).type === "cenotaph" && meta(135).tier === 1, "Cenotaph +1");
assert(meta(150).type === "selene" && meta(150).tier === 1, "Selene +1");
...
assert(codec.indexOf('"cenotaph", "kaleido", "helios", "selene"') >= 0, "netcodec ids");
assert(codec.indexOf("myrmidon") < 0 && codec.indexOf("harrow") < 0 && codec.indexOf('"nexus"') < 0, "old codec ids gone");
assert(/var VER = 4;/.test(codec), "codec VER unchanged (type names at same indices)");

assert(pvp.indexOf("cenotaph:") >= 0 && pvp.indexOf("helios:") >= 0 && pvp.indexOf("selene:") >= 0, "pvp kits");
